...
const NavCollapse = ({ menu, level }) => {
  ...
  const menuIcon = menu.icon ? (
    <Icon
      ...
      className={`group-hover:text-[#0118c4] ${selected==menu.id ? ' text-white group-hover:text-white':""} `}
      // sx={{
      //   '&.Mui-selected': {
  ...
  return (
    <>
      <ListItemButton
        className={`group text-gray-400 hover:text-[#0118c4]  rounded-[10px]`}
        sx={{
          ...
          pl: `${level * 24}px`,
          '&.Mui-selected': {
            backgroundColor: '#4535C1', // Light blue background when selected
            color: '#fff',
            '&:hover': {
              backgroundColor: '#4535C1',
              color: '#fff' // Slightly darker blue on hover
