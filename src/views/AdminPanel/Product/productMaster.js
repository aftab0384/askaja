...
const columns = [
  ...
  { id: 'productPrice', label: 'Price', minWidth: 150, align: 'center' },
  { id: 'productCategory', label: 'Category', minWidth: 150, align: 'center' },
...
export const ProductMaster =  () => {
  ...
  const [proPriceErr, setProPriceErr] = useState(false);
  ...
  const handelAddProduct = () => {
    if (ProductForm.productName !== '' && ProductForm.productPrice !== '') {
      setIsButtonLoading(true);
      const body = {
        productName: ProductForm.productName.trim(),
        productPrice: ProductForm.productPrice.trim(),
        description: ProductForm.description,
        categoryId: ProductForm.categoryId
      };
      axios
        .post(`${cafeBackend}/app/v1/product/addProduct`, body)
        .then((res) => {
          setIsButtonLoading(false);
          toast.success(res.data.result || 'Product Added Successfully');
          ClearField();
          handleClose();
        })
        .catch((err) => {
  ...
  return (
    <>
            ...
            {addUpdateModal.addModal && !addUpdateModal.updateModal && (
              <div className="flex flex-col gap-5">
                <FormControl fullWidth>
                  ...
                  />
                  {proNameErr && <p className="text-red-500 text-xs ml-2">Product name is required</p>}
                </FormControl>
                <FormControl fullWidth>
                  <TextField
                    label="Price"
                  ...
                  />
                  {proPro && <p className="text-red-500 text-xs ml-2">Product name is required</p>}
                </FormControl>
                <FormControl fullWidth>
                  <TextField
                    label="Description"
      ...
      <Modal open={isMdlOpen} onClose={handleMdlClose} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <Box sx={style} className=" w-96 max-lg:w-96 max-md:w-56 h-96 max-lg:h-60 max-md:h-56  p-4">
          <div className=" max-lg:w-full flex flex-col gap-1 bg-white my-4 p-4 rounded-xl ">
            <p className="font-semibold text-center">Are You Sure to delete?</p>
            <div className="flex justify-around pb-5">
              <Button variant="contained" className="bg-blue-700" onClick={handleDeleteCategory}>
                Yes
              </Button>
              <Button variant="outlined" color="error" onClick={handleMdlClose}>
                No
              </Button>
            </div>
          </div>
